...
class RoomSettings {
	...
	tourStatus() {
		if (!this.user.can('tournamentsmanagement', null, this.room)) return this.button(this.room.toursEnabled ? '+' : '#', true);

		if (this.room.toursEnabled) {
			return this.button('+', true) + this.button('#', null, 'tournament disable');
		} else {
			return this.button('+', null, 'tournament enable') + this.button('#', true);
		}
	}
	blacklist() {
		if (!this.user.can('editroom', null, this.room)) return this.button(this.room.blacklistPermission === '@' ? '@' : '#', true);

		if (this.room.blacklistPermission === '@') {
			return this.button('@', true) + this.button('#', null, 'blacklistpermission #');
		} else {
			return this.button('@', null, 'blacklistpermission @') + this.button('#', true);
		}
	}
	uno() {
