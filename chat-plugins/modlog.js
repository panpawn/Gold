...
function prettifyResults(resultArray, roomid, searchString, exactSearch, addModlogButton, hideIps, maxLines) {
	if (resultArray === null) {
...
function getModlog(connection, roomid = 'global', searchString = '', maxLines = 20, timed = false) {
	...
	PM.query({cmd: 'modlog', roomidList, searchString, exactSearch, maxLines}).then(response => {
		connection.send(prettifyResults(response, roomid, searchString, exactSearch, addModlogButton, hideIps, maxLines));
