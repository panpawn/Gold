...
const commands = {
	...
	*/
	...
	roomauth(target, room, user, connection, cmd) {
		...
		let buffer = Object.keys(rankLists).sort((a, b) =>
			...
			roomRankList = roomRankList.map(s => {
				...
				return s in targetRoom.users && !isAway ? Gold.nameColor(s, true) : Gold.nameColor(s);
			});
	...
	lock(target, room, user, connection, cmd) {
		let week = true; //cmd === 'wl' || cmd === 'weeklock';
		...
		Gold.removeAllMoney(userid, user.name);
		return true;
	},
	lockhelp: [
		`/lock OR /l [username], [reason] - Locks the user from talking in all chats for a week. Requires: % @ * & ~`,
	...
	unbanip(target, room, user) {
		...
		if (!this.can('rangeban')) return false;
		if (!Punishments.ips.has(target) || !Users.bans[target]) {
	...
	blacklisthelp: [
		`/blacklist [username], [reason] - Blacklists the user from the room you are in for a year. Requires: # & ~ (@ settable via /roomsettings)`,
		`/unblacklist [username] - Unblacklists the user from the room you are in. Requires: # & ~ (@ settable via /roomsettings)`,
	...
	async hotpatch(target, room, user) {
		...
		try {
			if (target === 'all') {
				...
				Chat.uncacheDir('./server/gold-plugins');
