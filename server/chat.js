	...
	canTalk(message = null, room = null, targetUser = null) {
		...
		if (!this.checkBanwords(room, message) && !user.can('mute', null, room)) {
			if (room.id === 'lobby' && !user.can('hotpatch')) {
				...
				room.add(user.name + " was automatically muted by the server for " + muteTime + " minutes. (Your message contained a banned word in this room.)").update();
				return false;
			} else {
...
Chat.sendPM = function (message, user, pmTarget, onlyRecipient = null) {
	const emotes = (Gold.emoticons.processEmoticons(message) !== message ? Gold.emoticons.processEmoticons(Chat.escapeHTML(message)) : false);
	const originalMessage = message;
...
Chat.loadPlugins = function () {
	...
	let goldFiles = FS('server/gold-plugins/').readdirSync();
