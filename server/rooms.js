...
class GlobalRoom extends BasicRoom {
	...
	startLockdown(err = null, slow = false) {
		...
		for (const user of Users.users.values()) {
			user.send(`|pm|~|${user.group}${user.name}|/raw <div class="broadcast-red"><b>The server is restarting soon.</b><br />Please take your time to finish your battles. No new battles can be started until the server resets in a few minutes.</div>`);
		}
...
class BasicChatRoom extends BasicRoom {
	...
	constructor(roomid, title, options = {}) {
		...
		this.deleteInactive = setTimeout(function () {
			if (this.id !== 'global' && this.type !== 'battle' && !this.protect && !this.isOfficial && !this.isPrivate && !this.isPersonal && !this.isStaff && this.messageCount < 40) {
				...
				this.add('<strong class="message-error">This room has been automatically deleted due to inactivity.  It will be removed upon the next server restart.</strong>').update();
				this.modchat = '~';
				Rooms('staff').add(`|raw|<strong class="message-error">${this.title} has been automatically deleted from the server due to inactivity.</strong>`).update();
			}
	...
	getIntroMessage(user) {
		let message = ``;
		let postMessage = Chat.html`\n|raw|<div class="infobox"> You joined ${this.title}`;
		if (this.modchat) {
			postMessage += ` [${this.modchat} or higher to talk]`;
		}
		if (this.modjoin) {
			let modjoin = this.modjoin === true ? this.modchat : this.modjoin;
			postMessage += ` [${modjoin} or higher to join]`;
		}
		if (this.slowchat) {
			postMessage += ` [Slowchat ${this.slowchat}s]`;
		}
		postMessage += `</div>`;
	...
	onConnect(user, connection) {
		...
		const title = this.title2 ? this.title2 : this.title;
