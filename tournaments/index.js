	...
	if (cmd === '') {
		...
		if (tour) {
			...
			if (room.tourAnnouncements) {
				...
				if (tourRoom && tourRoom !== room) tourRoom.addRaw('<div class="infobox" style="font-size: 1.5em;"><a href="/' + room.id + '" class="ilink"><strong style="color: #ff9900;">' + Tools.escapeHTML(Tools.getFormat(tour.format).name) + '</strong> tournament created in <strong>' + Tools.escapeHTML(room.title) + '</strong>.</a></div>').update();
			}
		}
	} else {
